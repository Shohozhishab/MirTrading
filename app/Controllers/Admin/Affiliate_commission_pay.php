...
class Affiliate_commission_pay extends BaseController
{
    ...
    public function index()
    {
        ...
        if (!isset($isLoggedIn) || $isLoggedIn != TRUE) {
            ...
            $table = DB()->table('commission_pay');
            $table->where('sch_id',$shopId);
            if (!empty($data['affiliate_user_id'])){
                $table->where('affiliate_user_id',$data['affiliate_user_id']);
            }
            elseif (!empty($data['st_date'])){
                $table->where('date >',$data['st_date']);
            }
            elseif (!empty($data['affiliate_user_id'])){
                $table->where('date <',$data['en_date']);
            }
            ...
            if (isset($data['mod_access']) and $data['mod_access'] == 1) {
                echo view('Admin/Affiliate_commission_pay/list', $data);
            } else {
